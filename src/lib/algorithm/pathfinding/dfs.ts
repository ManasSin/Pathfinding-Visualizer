...
export const dfs = (grid: GridType, startTile: TileType, endTile: TileType) => {
  ...
  const unTraversedTile = [base];

  while (unTraversedTile.length > 0) {
    const currentTile = unTraversedTile.pop();
    if (currentTile) {
      ...
      for (let i = 0; i < neighbors.length; i++) {
        if (!checkStack(neighbors[i], unTraversedTile)) {
          neighbors[i].distance = currentTile.distance + 1;
          neighbors[i].parent = currentTile;
          unTraversedTile.push(neighbors[i]);
        }
      }
    }
  }
